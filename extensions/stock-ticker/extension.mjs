...
// request_summary: the canvas marks the summary card "thinking" and returns a
// prompt built from the live quotes; answer it SILENTLY with the host model and
// write the prose back via set_summary. Runs in the background so the POST
// returns immediately (the spinner is the server-pushed pending state). Keyed
// per watchlist domain so a double-click can't fire twice.
wrapAction("request_summary", (original) => async (api) => {
  ...
  const key = `summary:${api?.ctx?.domainId ?? "default"}`;
  if (!claim(key)) return result;
  if (!session) {
    Promise.resolve()
      .then(() => runtime?.invokeFromAgent("fail_summary", { message: "No active Copilot session. Try again." }, back))
      .catch((e) => console.error(`[stock-ticker] fail_summary errored: ${e?.message ?? e}`))
      .finally(() => release(key));
    return result;
  }
  Promise.resolve()
    .then(() => host.ai(prompt))
    .then((text) => {
      if (!text) throw new Error("the analyst returned an empty summary");
      return runtime.invokeFromAgent("set_summary", { text }, back);
    })
    .catch((e) => {
      console.error(`[stock-ticker] ai summary failed: ${e?.message ?? e}`);
      return runtime?.invokeFromAgent("fail_summary", { message: String(e?.message ?? e) }, back).catch(() => {});
    })
    .finally(() => release(key));
  return result;
});
